...
func LoadSpecFile(filename string) (SpecFile, hcl.Diagnostics) {
	...
	if err != nil {
		return SpecFile{}, hcl.Diagnostics{
			&hcl.Diagnostic{
				Severity: hcl.DiagError,
				Summary:  "Cannot open spec file",
				Detail:   fmt.Sprintf("Failed to open specification file: %s.", filename),
			},
		}
	}
	return LoadSpecFileContent(src, filename)
}
...
func LoadSpecFileContent(src []byte, filename string) (SpecFile, hcl.Diagnostics) {
	...
	if diags.HasErrors() {
		return SpecFile{RootSpec: errSpec}, diags
	}
...
func decodeSpecBlock(block *hcl.Block) (hcldec.Spec, hcl.Diagnostics) {
	...
	case "tuple":
		return decodeTupleSpec(block.Body)
...
func decodeTupleSpec(body hcl.Body) (hcldec.Spec, hcl.Diagnostics) {
	content, diags := body.Content(specSchemaUnlabelled)
